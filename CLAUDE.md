...
- **`cmd_vel_to_joy.py` は実パッド用パイプライン（`pscon_node`/`can_node`、torobo2026_ros2_rp）にそのまま相乗りする設計**。それらのノードは変更せず、`/joy` の中身（axes/buttons）を実パッドが送るものと同じ形に合わせて出力することで、スマホからの操作を実パッドの操作と区別なく扱わせる。**注意**: 同リポジトリには`ps4con_node`/`can_ps4_node`という似た名前の別ノード一式もある。これは実PS4パッドを直挿しした場合の非常用バックアップ経路として意図的に残されているもの（ボタン配置は×○△□L1 R1 L2 R2で、`pscon_node`/`can_node`側のXbox系配置 A B X Y LB RB LTとは別物。両者は同時には使わない）。基本の操縦は本コントローラ→`pscon_node`/`can_node`（`firmware/CAN_COMMAND_SPEC.md`参照）。
...
| `/robot/command` | `std_msgs/msg/String` | 端末→機体 | 半自動トリガー。全機構コマンドは「1ボタン=定型アーム動作起動」のエッジ（モーメンタリ、ホールド無し）で統一。実Xbox系パッドのボタン配置（A B X Y LB RB LT RT）に1:1対応: `release_suction`(A,吸引切る/設置＝放す) / `intake`(B,回収) / `launch`(Y,射出) / `arm_start`(X,初期/リセット) / `arm_force_stop`(LB,アーム移動強制ストップ) / `manual_adjust`(RB,手動位置調整/ベル直位置調整) / `descend_adjust`(LT,降下微調整) / `dpad_up`(十字上,ベル直設置) / `dpad_right`(十字右,2個めボール置き場) / `dpad_left`(十字左,城門設置高さ) / `dpad_down`(十字下,関所設置高さ) / `estop` / `release`。**例外**: `lidar_correct`(RT,Lidar位置補正起動)のみホールド式。UI側が押している間だけ`BUTTON_PULSE_S`(0.15s)より短い間隔で連打してパルスを継続更新することで擬似的にホールドを実現する（Lidarの精度がまだ低いため常時自動発火ではなくボタンでゲートする仕様。`can_node.cpp`の`lider_callback()`参照）。詳細は`firmware/CAN_COMMAND_SPEC.md`参照。 |
...
| `/joy` | `sensor_msgs/msg/Joy` | ROS2→チーム側 | `/cmd_vel`(軸)と`/robot/command`(ボタン)をマージして20Hz固定で publish。**実パッドが `pscon_node`(`torobo2026_ros2_rp`)に送るものと同じ形**に合わせる: `axes[5]`=linear.x(st_ry), `axes[4]`=linear.y(st_rx), `axes[6]`=angular.zの符号のみ(-1/0/1, cross_bt), `axes[7]`=angular.zの大きさ(0.0〜1.0、旋回速度。pscon_node/can_node/STM32側で実際の旋回速度に反映)。`buttons[0..7]`は`/robot/command`のid→index(`cmd_vel_to_joy.py`の`COMMAND_BUTTON_MAP`定数。`release_suction`→0, `intake`→1, `launch`→2, `arm_start`→3, `arm_force_stop`→4, `manual_adjust`→5, `descend_adjust`→6, `lidar_correct`→7)に対応するindexが受信後約150ms(3フレーム)だけ1になるエッジパルス（`lidar_correct`のみUI側が連打してホールドを疑似再現）。十字4方向(`dpad_up/right/left/down`)は`DPAD_AXIS_MAP`で`axes[0..3]`に同様のエッジパルスとして出る。`axes`は8要素固定、`buttons`は8要素固定。**安全ゲート**: 最後の`/cmd_vel`から200ms超のデッドマン、および`estop`/`release`によるラッチで、いずれの停止条件でもaxes・buttons(全要素)を全0にする。CAN側の実際の解釈は`firmware/CAN_COMMAND_SPEC.md`参照。 |
